...
export class SwaggerStrategy implements ScraperStrategy {
  async scrape(url: string): Promise<ApiDocument> {
    try {
      return await withChromiumPage(async (page) => {
        ...
        const toParamIn = (raw: string): 'query' | 'header' | 'path' | 'cookie' | 'body' => {
        ...
        const serverSelect = document.querySelector('.servers select') as HTMLSelectElement;
        if (serverSelect) {
        ...
        opblocks.forEach((block) => {
            ...
            if (bodySection) {
                 const contentTypeSelect = bodySection.querySelector('.content-type') as HTMLSelectElement;
        ...
        return extractedDoc as unknown as ApiDocument;
      });
    } catch (error) {
