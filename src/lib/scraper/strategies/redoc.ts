...
import { ApiDocument, ScraperStrategy, ApiEndpoint, ApiParameter, ApiProperty, ApiResponse, ApiRequestBody } from '../../types';
import { withChromiumPage } from '../playwright-context';

export class RedocStrategy implements ScraperStrategy {
    ...
    try {
      return await withChromiumPage(async (page) => {
        ...
        $('span[class^="ServerUrl"]').each((_, el) => {
          ...
          if (url) servers.push({ url, description: '' });
        });
        ...
        $('[data-section-id]').each((_, opElement) => {
          ...
          if (!sectionId || !sectionId.includes('operation/')) return;

          const httpVerbEl = $(opElement).find('.http-verb, .operation-type').first();
          if (httpVerbEl.length === 0) return;
          ...
          $(opElement).find('h5').each((_, h5El) => {
              ...
              if (h5Text.includes('parameters')) {
                  let inType: 'query' | 'header' | 'path' | 'cookie' | 'body' = 'query';
                  if (h5Text.includes('header')) inType = 'header';
                  if (h5Text.includes('path')) inType = 'path';
                  if (h5Text.includes('cookie')) inType = 'cookie';
