 ...
 */
export function detectDocsType(htmlContent: string, _url: string): DocsType {
  const htmlLower = htmlContent.toLowerCase();

  // Swagger UI Detection
  if (
    htmlLower.includes('id="swagger-ui"') ||
  ...
  if (
    htmlLower.includes('name="generator" content="docusaurus"') ||
